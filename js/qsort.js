	...
	jQuery.fn.qsort = function(o) {
		...
		try{
			...
			jQuery(values).each(function() {
				var loc = -1;
				if(o.digits==true) {
					loc = jQuery.inArray(parseInt(this.toString()), oldValues);
				}else {
					loc = jQuery.inArray(this.toString(), oldValues);
				}
			...
			if(o.order == "desc") {
				for(i = 0; i < oldValues.length - 1; i++) {
					jQuery(sortedElems[i]).before(jQuery(sortedElems[i+1]));			
				}	
			}else {
				for(i = 0; i < oldValues.length - 1; i++) {
					jQuery(sortedElems[i]).after(jQuery(sortedElems[i+1]));			
				}			
			}
			return jQuery(this);
		}catch(e) {
